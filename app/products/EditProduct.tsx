...
export default function EditProduct({ setAddNew, productsArray }: Props) {
  ...
  const handleProductChange = (
    ...
    if (value !== "select") {
      const result = productsArray.filter(
        (item: Product) => item.name === value
      );
      list[i]["price"] = result[0].price;
      list[i]["amount"] = (
        Number(list[i].price) * Number(list[i].quantity)
      ).toLocaleString();
    }
    list[i][name] = value;
    setProducts(list);
          ...
          {products.map((product, index) => (
            <div key={index}>
              <div className="w-full flex sm:flex-row flex-col md:space-x-4 mb-2">
                ...
                <div className=" w-full mb-2">
                  <div className="flex flex-col mb-2">
                    ...
                    <p className="border-[1px] py-2 px-4 rounded text-sm">
                      {Number(product.price).toLocaleString()}
                    </p>
                  </div>
                  <div className="flex flex-col mb-2">
                    <p className="text-sm">Amount</p>
                    <p className="border-[1px] py-2 px-4 rounded text-sm bg-gray-100">
                      {product.amount}
                    </p>
                  </div>
                </div>
              ...
              {products.length - 1 === index && (
                <IoMdAddCircle
                  className="text-green-500 rounded bg-white text-3xl cursor-pointer mb-8"
                  onClick={addProduct}
                />
              )}
            </div>
          ))}
          <button className="bg-blue-500 rounded text-white py-2 px-4">
            RECORD
          </button>
        </form>
      </div>
